                ...
                <table class="table table-striped table-hover r-0">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in itemsBerkas" :key="index" :class="{ 'red lighten-3' : item.tanggal_pengukuran <= 0 && item.status_proses == 'proses' }">
                            <td class="pl-md-5" width="80%">
                                <div class="avatar avatar-md mr-3 mt-1 float-left">
                                    <img class="w-80px mb-4" src="/goukur/img/dummy/u2.png" alt="">
                                </div>
                                <div>
                                    <div>
                                        <strong>{{item.nama_pemohon}}</strong>
                                    </div>
                                    <small class="d-none d-md-block">Petugas: {{item.petugas_ukur}} | No. Telepon: {{item.petugas_ukur_nomor}}
                                    </small>
                                </div>
                            </td>
                            <td align="right">
...
export default {
    ...
    methods: {
        filterStatus(param) {
            axios.
            get('/berkas-ppat', {params: {filterData: param}})
                .then(response => {
